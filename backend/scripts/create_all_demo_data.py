...
def create_demo_customers(db: Session, count: int = 20):
    """Create a single demo customer"""
    print(f"Creating {count} demo customer...")
    ...
    for i in range(count):
        ...
        customer = Customer(
            ...
            occupation=random.choice(
                ["Engineer", "Manager", "Director", "Consultant", "Business Owner"]
            ),
...
def create_demo_categories(db: Session):
    """Create a single demo product category"""
    print("Creating demo category...")
...
def create_demo_brands(db: Session):
    """Create a single demo product brand"""
    print("Creating demo brand...")

    brands_data = [
        ("Tijaero", "TJ", "Tijaero Default Brand"),
    ]

    brands = []
    for name, code, desc in brands_data:
        brand = ItemsBrand(brand_name=name, brand_code=code, description=desc)
        brands.append(brand)
...
def create_finance_demo_data(db: Session, customers):
    ...
    for i in range(1):
        ...
        deposit = BankDeposits(
            deposits_amount=Decimal(random.uniform(100, 10000)).quantize(
                Decimal("0.01")
            ),
    ...
    for i in range(1):
        ...
        advance = CustomerAdvancePayments(
            ...
            branch_code=random.choice(branches),
            payment_amount=Decimal(random.uniform(500, 10000)).quantize(
                Decimal("0.01")
            ),
...
def main():
    ...
    try:
        # Create base data
        customers = create_demo_customers(db, count=1)
        categories = create_demo_categories(db)
        brands = create_demo_brands(db)
        products = create_demo_products(db, categories, brands, count=1)
