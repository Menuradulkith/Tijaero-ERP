...
from app.auth.models import Branch, Group, User
...
def create_demo_users():
    ...
    try:
        ...
        for user_data in demo_users:
            ...
            existing_employee = (
                db.query(Employee)
                .filter(Employee.employee_id == user_data["employee_id"])
                .first()
            )
            ...
            user = User(
                ...
                gender=(
                    "Male"
                    if user_data["first_name"] in ["John", "Mike", "David", "James"]
                    else "Female"
                ),
            ...
            # Print success
            branch_names = [
                branches[idx].branch_code
                for idx in user_data["branches"]
                if idx < len(branches)
            ]
        ...
        for user_data in demo_users:
            print(
                f"   {user_data['username']:20} | {user_data['password']:15} | {user_data['role']}"
            )
