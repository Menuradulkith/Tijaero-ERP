...
class ItemTransferNote(Base):
    ...
    from_location = relationship(
        ...
        back_populates="item_transfer_notes_from",
    )
    to_location = relationship(
        "Locations",
        foreign_keys=[to_location_id],
        back_populates="item_transfer_notes_to",
    )
    ...
    item_products = relationship(
        "ItemTransferNoteItemProduct", back_populates="item_transfer_note"
    )
    approved_records = relationship(
        "ItemTransferNoteApproved", back_populates="item_transfer_note"
    )
...
class ItemTransferNoteItems(Base):
    ...
    barcode = Column(Text)
    branch_code = Column(String(200), nullable=False)
    itemtransfernote_id = Column(
        Integer, ForeignKey("item_transfer_note.id"), nullable=False
    )
...
class ItemTransferNoteItemProduct(Base):
    ...
    item_transfer_note = relationship(
        "ItemTransferNote", back_populates="item_products"
    )
...
class ItemTransferNoteApproved(Base):
    ...
    approved_user_id = Column(Integer, ForeignKey("accounts_user.id"))
    approved_date = Column(TIMESTAMP)

    # Relationships
    item_transfer_note = relationship(
        "ItemTransferNote", back_populates="approved_records"
    )
