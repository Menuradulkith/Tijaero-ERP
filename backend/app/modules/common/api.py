...
def get_reference_data(
    ...
    if "branches" in includes:
        ...
        result["branches"] = [
            {"id": b.id, "branch_code": b.branch_code, "branch_name": b.branch_name}
            for b in branches
        ]
    ...
    if "categories" in includes:
        ...
        categories = category_service.get_all_categories(
            db, skip=0, limit=1000, active_only=False
        )
        result["categories"] = [
            {"id": c.id, "name": c.name, "active": c.active} for c in categories
        ]
    ...
    if "locations" in includes:
        ...
        result["locations"] = [
            {"id": l.id, "name": l.name, "branch_code": l.branch_code}
            for l in locations
        ]
...
def get_pending_approvals(
    approval_type: Optional[str] = Query(
        None,
        description="Filter by type: sales_order, sale_return, purchase_return, etc.",
    ),
