...
class NotificationService:
    ...
    def create_notification(self, notification: schemas.NotificationCreate) -> models.UserNotification:
    ...
    def get_user_notifications(
        ...
        return query.order_by(desc(models.UserNotification.created_date)).offset(skip).limit(limit).all()

    def mark_as_read(self, notification_id: int, user_id: int) -> models.UserNotification:
        """Mark notification as read"""
        notification = self.db.query(models.UserNotification).filter(
            and_(
    ...
    def mark_all_as_read(self, user_id: int) -> int:
        ...
        count = self.db.query(models.UserNotification).filter(
            and_(
                models.UserNotification.user_id == user_id,
    ...
    def get_notification_stats(self, user_id: int) -> schemas.NotificationStats:
        ...
        total = self.db.query(func.count(models.UserNotification.id)).filter(
            models.UserNotification.user_id == user_id
        ).scalar() or 0
        
        unread = self.db.query(func.count(models.UserNotification.id)).filter(
            and_(
                models.UserNotification.user_id == user_id,
                models.UserNotification.is_read == False
            )
