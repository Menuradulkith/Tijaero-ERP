from sqlalchemy import Column, Integer, String, Text, ForeignKey, Date, Boolean, TIMESTAMP, BigInteger
...
class CustomerSupport(Base):
    ...
    assigned_user_id = Column(BigInteger, ForeignKey("users.id"), nullable=False)
...
class CSJobItem(Base):
    ...
    comment = Column(Text)
    customer_support_id = Column(Integer, ForeignKey("customer_support.id"), nullable=False)
