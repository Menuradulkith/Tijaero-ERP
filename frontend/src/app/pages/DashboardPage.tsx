...
export default function DashboardPage() {
  ...
  const { metrics, loading, error, refresh } = useDashboardMetrics({
    autoRefresh: canViewDashboard, // Only auto refresh if they have permission
    skipInitialFetch: !canViewDashboard, // Don't fetch on mount if they lack permission
  });
  ...
  // Show error state (but only if they have permission to view it)
      ...
      <Box mt={4}>
        <TSection title="Quick Actions" marginBottom={0}>
          <Grid container spacing={2}>
            ...
            {hasPermission(
              user,
              PERMISSIONS.FINANCE_CREATE.resource,
              PERMISSIONS.FINANCE_CREATE.action,
            ) && (
              <Grid item xs={6} sm={3}>
                <Card
                  sx={{
